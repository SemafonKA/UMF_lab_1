   ...
   for (std::size_t i = 0; i < grid.sizeY; i++) {
      for (std::size_t j = 0; j < grid.sizeX; j++) {
         ...
         case NodeType::secondBounary:
            throw std::exception("Second boundaries is not implemented yet");
         ...
         case NodeType::normal: {
            ...
            bool isUniformX = (h_x[0] == h_x[1]);
            bool isUniformY = (h_y[0] == h_y[1]);

            auto& diags = mat.diags;

            // Нижний элемент
            if (isUniformY) {
               diags[0].at(linearPos) = -(doubledLambda / (2.0 * h_y[0] * h_y[0]) - lambdaDify / (2.0 * h_y[0]));
            }
            else {
            ...
            // Левый элемент
            if (isUniformX) {
               diags[1].at(linearPos) = -((doubledLambda / (2.0 * h_x[0] * h_x[0])) - lambdaDifx / (2.0 * h_x[0]));
            }
            else {
            ...
            // Правый элемент
            if (isUniformX) {
               diags[3].at(linearPos) = -((doubledLambda / (2.0 * h_x[0] * h_x[0])) + lambdaDifx / (2.0 * h_x[0]));
            }
            else {
            ...
            // Верхний элемент
            if (isUniformY) {
               diags[4].at(linearPos) = -(doubledLambda / (2.0 * h_y[0] * h_y[0]) + lambdaDify / (2.0 * h_y[0]));
            }
            else {
